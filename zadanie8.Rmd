...
Nie je to nejako extra esteticky napísaný kód, ale nechcelo sa mi s tým výrazne babra». Ak tam chcete dosadi» svoje dáta, tak iba vymeòte vstupný dataset.

Ako prvý preventívne spú¹»am tidyverse:
```{r}
library(tidyverse)
```
...
Vypoèítam èleny, ktoré pôjdu ako sèitatele do "integrálu": 
```{r}
...
Rozdelím si datasety na podskupiny podµa h:
```{r}
...
Vypoèítam èitateµa a menovateµa jednotlivých Q_h:
```{r}
...
Dám ich do tabuµky a zostrojím grafy (tie tie¾ nie sú estetické, ale natrez to staèí :-) ):
```{r}
